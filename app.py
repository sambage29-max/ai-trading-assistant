                    ...
                    if sl != "N/A":
                        ...
                        if risk_per_unit > 0:
                            ...
                            if segment_selector == "Option Chains":
                                ...
                                st.success(f"⚖️ **Position Size Calculator:** Max Rupee Risk Allowed = **₹{max_risk_cash:.2f}**. Recommended Derivative Size = **{recommended_lots * lot_size} Units ({recommended_lots} Lots)**.")
                            else:
                                st.success(f"⚖️ **Position Size Calculator:** Max Rupee Risk Allowed = **₹{max_risk_cash:.2f}**. Recommended Equity Size = **{calculated_qty} Shares**.")
                        else:
                            st.warning("⚖️ **Position Size Calculator:** Stop Loss width is too tight to securely size risk metrics.")
                    else:
                        st.info("⚖️ **Position Size Calculator:** Waiting for a breakthrough mathematical confirmation signal.")
